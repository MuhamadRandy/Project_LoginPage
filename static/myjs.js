...
function num2str(count) {
  if (count > 10000) {
    // jika angka real = 1000 maka diubah ke 100 lalu dikecilkan ke 1 dan ditambah k
    return parseInt(count / 1000) + 'k';
  }

  if (count > 500) {
    // jika angka real = 500 maka dibagi 100 = 5 lalu dibagi 10 = 0,5 dan ditambah k
    return parseInt(count / 100) / 10 + 'k';
  }

  if (count == 0) {
    // jika angka 0 maka tampilkan hasil kosong
    return '';
  }
  return count;
}
    ...
    success: function (response) {
      if (response["result"] === "success") {
        ...
        for (let i = 0; i < posts.length; i++) {
          ...
          let class_thumbsup = post["thumbsup_by_me"] ? "fa-thumbsup-up" : "fa-thumbs-o-up";
...
function toggle_thumbsup(post_id, type) {
  ...
  if ($i_like.hasClass("fa-thumbs-up")) {
    ...
    $.ajax({
      ...
      success: function (response) {
        console.log("like");
        $i_like.addClass("fa-thumbs-up").removeClass("fa-thumbs-o-thumbs");
        $a_like.find("span.like-num").text(num2str(response["count"]));
      },
    });
  }
